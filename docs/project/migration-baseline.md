...
| 文件 | 内容 |
| --- | --- |
| `V001__baseline.sql` | 全部 Schema：表、列、约束、索引，以及每张表、每一列的中文 Catalog 注释 |
| `V002__built_in_roles.sql` | 内置角色与权限码种子数据 |

新增变更从 `V003` 起分配；[数据库版本与迁移规范](../standards/development/database-version-and-migration-guidelines.md)
第 4 节的默认规则：已共享 Migration 不修改、删除、重命名，错误用更高版本前向修复。

注释与表定义放在一起：每张表在 `V001` 里是"建表 → 表注释 → 按列顺序的逐列注释 → 该表索引"，
不另开只写注释的迁移，读一个文件就能看到结构与字段含义。
...
- 收缩完成后在 `V001` 内补齐 Catalog 注释：原先 83 列里只有 14 列有注释，现按要求逐列补齐到 83 列，
  并把两处已失效的表注释（`admin_workspace` 写"一对一映射"、`admin_role_permission` 写 `模块:*`）
  改写为当前语义。这两处属于同一次授权的基线重置收尾，不额外开前向迁移。
...
| 新链 `V001`–`V002`（结构与种子） | 10 张表、83 列、103 条约束、24 个索引与两张种子表与旧链完全一致 |
| 新链 `V001`–`V002`（注释） | 与旧链的差异只有 69 条新增列注释与 2 条修正后的表注释，没有结构性或数据性差异 |
...
- 重置窗口在 `V001` 补齐 Catalog 注释后关闭：至此 `V001`、`V002` 冻结，之后任何改动（含注释）
  都必须从 `V003` 起新增前向迁移；
