...
平台侧的完整接入标准保存在平台仓库（私有），按需授权查阅；这里不复述，也不暴露平台内部实现。
...
4. **工作区映射**：平台实例与本地工作区一对一，且业务表都带 workspace 维度。
...
- 验证用户上下文 Token 的 `iss`、`aud`、`kid`、签名与 `exp`；未知 `kid` 只触发一次受限的公钥刷新，
  不允许跳过验签；
- 本地会话的绝对过期时间**不晚于** Token 的 `exp`，首版不提供刷新令牌或滑动续期；
...
出现真实用例时按平台仓库的完整接入标准逐项启用，**不要**在模板里预留空实现或空接口。
