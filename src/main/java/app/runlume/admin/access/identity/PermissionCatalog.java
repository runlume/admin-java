 ...
 * 后台权限码目录与通配匹配规则。
 *
 * <p>权限码约定与 admin-design 前端一致：{@code *} 表示全部，{@code 模块:*} 表示模块内全部，
 * 其余精确匹配。界面按原始权限码过滤菜单与按钮，服务端把 {@code *} 与 {@code 模块:*}
 * 展开成具体码后再写入 Spring Security 授权集合，避免出现“前端可见、后端拒绝”的偏差。</p>
 *
 * @author 树深技术
...
public final class PermissionCatalog {
    ...
    private static final List<PermissionDefinition> DEFINITIONS = List.of(
            ...
            new PermissionDefinition(USER_VIEW, "user", "查看账号"),
            new PermissionDefinition(USER_CREATE, "user", "新建账号"),
            new PermissionDefinition(USER_UPDATE, "user", "编辑账号"),
            new PermissionDefinition(USER_DISABLE, "user", "启用或禁用账号"),
     ...
     *
     * <p>返回集合不含 {@code *} 与 {@code 模块:*} 本身，未登记的权限码原样保留，
     * 便于业务域自行扩展。</p>
     *
     * @param granted 已授予的原始权限码
     * @return 展开后的具体权限码
